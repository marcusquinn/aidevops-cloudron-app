...
main() {
	jq -e '.manifestVersion == 2 and .version == "0.1.18" and .upstreamVersion == "3.32.294" and .minBoxVersion == "9.1.0" and .iconUrl != "" and .packagerName != "" and .packagerUrl == "https://github.com/marcusquinn" and (has("packageUrl") | not) and (.mediaLinks | length) > 0 and .changelog == "file://CHANGELOG"' \
	...
	assert_contains CHANGELOG '[0.1.18]' || return 1
	assert_contains CHANGELOG '* Update the bundled aidevops CLI to 3.32.294.' || return 1
	assert_contains CHANGELOG.md '## [0.1.18]' || return 1
	assert_contains CHANGELOG.md "- Updated and pinned the bundled aidevops CLI to \`3.32.294\`." || return 1
	...
	assert_contains Dockerfile 'ARG AIDEVOPS_VERSION=3.32.294' || return 1
