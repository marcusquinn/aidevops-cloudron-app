...
main() {
	jq -e '.manifestVersion == 2 and .version == "0.1.5" and .upstreamVersion == "3.32.181" and .minBoxVersion == "9.1.0" and .iconUrl != "" and .packagerName != "" and .packagerUrl == "https://github.com/marcusquinn" and (has("packageUrl") | not) and (.mediaLinks | length) > 0 and .changelog == "file://CHANGELOG"' \
	...
	assert_contains Dockerfile 'ARG AIDEVOPS_VERSION=3.32.181' || return 1
